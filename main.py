...
class LissajousWindow(qt.QMainWindow):
    def __init__(self):
        ...
        self.test_button.clicked.connect(self.test_button_click_handler)
    ...
    def test_button_click_handler(self):
        ...
        try:
            for i in range(number_of_tests):
                ...
                figure, params = self.generator.get_random_figure()
                self._ax.plot(figure.x_arr, figure.y_arr,
                              color=default_settings["color"],
                              linewidth=default_settings["width"])
                plt.axis("off")
                image_name = f"test_image_{i+1}"
                file_name = f"test_info_{i+1}.json"
                plt.savefig(os.path.join(folder_name, image_name))
